...
public class NachosSecurityManager extends SecurityManager {
	...
	if (perm instanceof FilePermission) {
	    if (perm.getActions().equals("read")) {
		// the src directory can only be read with privilege
		if (isPrivileged())
		    return;

		enablePrivilege();

		// not allowed to read src directory directly w/out privilege
		try {
		...
		// only allowed to write src diretory, and only with privilege
		verifyPrivilege();
